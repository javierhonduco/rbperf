...
use rbperf::{ProcessData, RubyStack, RubyVersionOffsets};
...
use rbperf::process::ProcessInfo;
use rbperf::profile::Profile;
use rbperf::ruby_readers::{
...
impl Rbperf {
    fn run(&mut self, sample_period: u64, duration: std::time::Duration) -> Profile {
        ...
        let mut versions = self.bpf.table("version_specific_offsets").unwrap();
        versions.set(&mut key.to_le_bytes(), &mut value).unwrap();

        // Set up the BPF programs
        let mut programs = self.bpf.table("programs").unwrap();
        let idx: i32 = 0;
        let val = self
            .bpf
        ...
        programs
            .set(&mut idx.to_le_bytes(), &mut val.to_le_bytes())
            .unwrap();

        // Set up perf events
        let table = self.bpf.table("events").unwrap();

        let mut perf_map = PerfMapBuilder::new(table, || self.perf_data_callback())
            .build()
            .unwrap();
        let _ = PerfEvent::new()
            .handler("on_event")
    ...
    fn add_pid(&self, pid: Pid) {
        // todo:
        // - add process to global structure
        // - error handling

        // Set the process configuration
        let mut pid_to_rb_thread = self.bpf.table("pid_to_rb_thread").unwrap();

        // Fetch process info
        let process_info = ProcessInfo::new(pid).expect("could not fetch ProcessInfo");
        ...
        pid_to_rb_thread
            .set(&mut pid.to_le_bytes(), &mut value)
            .unwrap();
    }
...
fn main() {
    let args = Command::new("rbperf")
        .arg(Arg::new("pid").takes_value(true))
        .get_matches();
    let pid: i32 = args
        .value_of("pid")
        .expect("A pid is required")
        .parse()
    ...
    r.add_pid(pid);
    let profile = r.run(sample_period, duration);
